...
RUN apk update
RUN apk add curl unzip which clang build-base git openjdk11 nodejs npm ttf-freefont nss chromium ruby
...
RUN cd tmp/app && gradle --no-daemon -i downloadDeps build run jacocoTestReport spotlessCheck && rspec --format RspecJunitFormatter --out rspec.xml && cat rspec.xml && cd / && rm -rf tmp && mkdir -p tmp
