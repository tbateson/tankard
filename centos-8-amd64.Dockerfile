...
RUN cd tmp/app && gradle --no-daemon -i downloadDeps build run jacocoTestReport spotlessCheck jacocoToCobertura && gradle --no-daemon -i --offline build run jacocoTestReport spotlessCheck jacocoToCobertura && cat build/reports/cobertura.xml && cd / && rm -rf /tmp && mkdir -p /tmp
