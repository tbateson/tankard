...
ARG ADOPTIUM_JDK_URL=https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.18%2B10/OpenJDK11U-jdk_x64_linux_hotspot_11.0.18_10.tar.gz
...
RUN cd tmp/app && gradle --no-daemon -i downloadDeps build run jacocoTestReport jacocoToCobertura && gradle --no-daemon -i --offline build run jacocoTestReport jacocoToCobertura && cat build/reports/cobertura.xml && cd / && rm -rf tmp && mkdir -p tmp
